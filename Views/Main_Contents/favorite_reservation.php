...
<script>
  ...
  function referrer(){
    ...
    var result = ref.match(/[favorite_detail.php]/);
    if(!result){
...
if($_SESSION['User']['referrer'] == 1 ) {
  ...
  //お気に入りテーブル（削除）＝＞予約テーブル（挿入）
  $reservations->reservation_favorite($favorite);
  $favorites->unregister();
